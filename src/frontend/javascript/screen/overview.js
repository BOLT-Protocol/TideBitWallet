...
class Overview {
  constructor() {}
  initialize(screen, tidewallet, fiat, version, { totalAsset, assets } = {}) {
    this.header = new Header(screen, { fiat, totalAsset });
    this.assetList = new AssetList(assets, fiat);
    this.settingList = new SettingList(tidewallet, fiat, version);
  ...
  render(screen, tidewallet, fiat, version, { totalAsset, assets } = {}) {
    const view = currentView();
    if (!view || (view !== "assets" && view !== "settings") || !this.scaffold) {
      this.initialize(screen, tidewallet, fiat, version, {
        totalAsset,
        assets,
      });
    } else {
